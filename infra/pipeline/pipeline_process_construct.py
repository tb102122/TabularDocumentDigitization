...
client = boto3.session.Session(profile_name="cardess-dev", region_name="eu-central-1").client

from aws_cdk import (core, aws_sqs, aws_lambda, aws_sns, aws_sns_subscriptions,
...
class PipelineProcessConstruct(core.Construct):
    ...
    def __create_stage_augment(self, stage):
        ...
        if len(commons['WORK_TEAM_NAMES']) == 0:
            raise Exception(
                'Make sure you have have added created work team name in the cdk.json: ENVIRONMENTS'
            )
        ...
        for workteam in commons['WORK_TEAM_NAMES']:
            if workteam not in work_teams:
                raise Exception(
                    f'Work team : {workteam} you have added does not exist in your account Make sure to '
            ...
            workflow_resource = A2IWorkflowConstruct(
                ...
                s3_output_path=self.__bucket.s3_url_for_object(
                    f'augment/.a2i'),
                workteam_arn=workteam_arn,
                task_count=1,
                task_description=
                f'Review Tables from Source Document - {workteam.title()}',
                task_title=f'Tabular Document Digitization')

            allowed_workflow_resource_arns.append(
                workflow_resource.get_workflow_arn())

            self.__stage_begin_lambdas[Process.AUGMENT].add_environment(
                f'WFLOW_A2I_{workteam.upper()}_ARN',
                workflow_resource.get_workflow_arn())
        ...
        human_review_event_pattern = aws_events.EventPattern(
            ...
            detail={'flowDefinitionArn': allowed_workflow_resource_arns})
    ...
    def __create_begin_lambda(self, stage, queue):
        ...
        lambda_function = self.__create_lambda_function(
            stage, Aspect.BEGIN, environ)
    ...
    def __create_lambda_function(self, stage, aspect, environ):
        ...
        lambda_function = aws_lambda.Function(
            ...
            code=aws_lambda.Code.from_asset(
                f'{self.__source}/processor/{stage}'),
